...
import { Plus, Globe, ExternalLink, RefreshCw, Trash2, Sparkles, Calendar, Loader2 } from "lucide-react";
...
export default function Sites() {
  ...
  // Auto-connect Shopify store when in Shopify mode
  const autoConnectShopifyMutation = useMutation({
    mutationFn: async (shopDomain: string) => {
      const shopUrl = `https://${shopDomain}`;
      
      // Create Shopify site connection (backend will use existing Shopify access token)
    ...
    onSuccess: (data) => {
      ...
      console.log("[SITES] Shopify store auto-connected", { siteId: data.id });
      
      // Auto-trigger crawl after connection
      if (data.id) {
        setTimeout(() => {
          crawlSiteMutation.mutate(data.id);
        }, 1000);
      }
    },
    onError: (error: any) => {
      console.error("[SITES] Failed to auto-connect Shopify store:", error);
      // Don't show error toast - this is automatic
    },
  });

  // Auto-connect Shopify store on mount if in Shopify mode
  useEffect(() => {
    if (isShopifyMode && shop && sites && !autoConnecting) {
      // Check if Shopify store is already connected
      const shopUrl = `https://${shop}`;
      const existingSite = sites.find(s => s.url === shopUrl || s.url.includes(shop));
      
      if (!existingSite) {
        console.log("[SITES] Auto-connecting Shopify store:", shop);
        setAutoConnecting(true);
        autoConnectShopifyMutation.mutate(shop);
      }
    }
  }, [isShopifyMode, shop, sites]);
  ...
  // Check if site belongs to current Shopify store
  const isShopifySite = (site: Site) => {
    return isShopifyMode && shop && (site.url.includes(shop) || site.url === `https://${shop}`);
  };
  ...
  // Filter sites in Shopify mode to only show the current store
  const displaySites = isShopifyMode && shop 
    ? sites?.filter(s => s.url.includes(shop))
    : sites;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {/* Hide "Connect Site" button in Shopify mode */}
        {!isShopifyMode && (
          <Button
      ...
      {!displaySites || displaySites.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-12">
            {autoConnectShopifyMutation.isPending ? (
              <>
                <Loader2 className="h-12 w-12 text-primary mb-4 animate-spin" />
                <h3 className="text-lg font-semibold mb-2">Connecting your Shopify store...</h3>
                <p className="text-muted-foreground text-center max-w-md">
                  Setting up your blog for SEO content generation
                </p>
              </>
            ) : (
              <>
                ...
                <p className="text-muted-foreground text-center mb-4 max-w-md">
                  {isShopifyMode 
                    ? "Your Shopify store will be automatically connected" 
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {displaySites.map((site) => {
            const isCurrentShopifySite = isShopifySite(site);
              ...
              <Card key={site.id} className="hover-elevate" data-testid={`site-card-${site.id}`}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="flex gap-2 pt-2">
                    ...
                    {/* Hide delete button for Shopify site in Shopify mode */}
                    {!isCurrentShopifySite && (
                      <Button
                        variant="outline"
                        size="sm"
      ...
      {/* Only show ConnectSiteDialog in standalone mode */}
      {!isShopifyMode && (
        <ConnectSiteDialog
